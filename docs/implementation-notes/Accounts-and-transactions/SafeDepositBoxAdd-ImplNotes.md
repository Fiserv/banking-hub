...
|`PartyAcctRelInfo.TaxReportingOwnerInd`|true<br>false|***Required** to create safe deposit box account and, there cannot be more than one tax relationships associated to a safe deposit box account. If primary tax reporting indicator is not provided, by default, first party will be considered as tax reporting owner.|
|`SafeDepositBoxInfo`|||
...
|`SafeDepositBoxInfo.PostAddr.OriginatingBranch`||***Required*****Required** if new address record is to be created and values in this field are user-defined.|
